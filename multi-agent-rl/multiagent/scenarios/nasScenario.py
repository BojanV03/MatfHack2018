...
class Scenario(BaseScenario):
    ...
    def adversary_reward(self, agent, world):
        ...
        # AKO DODJE DO CILJA
        aPos = np.array(agent.state.p_pos)
        lPos = np.array(world.landmarks[agent.index].state.p_pos)
        if np.linalg.norm(aPos - lPos) < world.landmarks[agent.index].size + agent.size:
            rew += 1000
            agent.active = False
            agent.accel = 0
            agent.max_speed = 0
